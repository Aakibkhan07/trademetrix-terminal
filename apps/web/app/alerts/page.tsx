    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        <div className="t-panel" style={{ padding: 16, flex: 1, maxWidth: 500 }}>
          <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 13, margin: '0 0 10px', color: 'var(--text)' }}>New Alert</h3>
          ...
          {msg && <p style={{ fontSize: 10, margin: '4px 0 0', color: msg.includes('symbol') ? 'var(--red)' : 'var(--green)' }}>{msg}</p>}
        </div>

        <div className="t-panel" style={{ padding: 16, width: 260 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 12, margin: 0, color: 'var(--text)' }}>Notifications</h3>
            <span style={{ fontSize: 9, color: notifChannels.length ? 'var(--green)' : 'var(--text-sub)' }}>
      ...
      {!loading && alerts.length === 0 && (
        <div className="t-panel" style={{ padding: 20, textAlign: 'center' }}>
          <p style={{ fontSize: 13, color: 'var(--text-faint)', margin: 0 }}>No alerts yet. Create one above.</p>
        </div>
      )}
      {alerts.map(a => (
        <div key={a.id} className="t-panel" style={{
          ...
          borderLeft: `3px solid ${a.is_active ? (a.triggered_at ? 'var(--amber)' : 'var(--violet)') : 'var(--text-faint)'}`,
        }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{
                ...
                background: a.condition === 'above' ? 'color-mix(in srgb, var(--green) 10%, transparent)' : 'color-mix(in srgb, var(--red) 10%, transparent)',
                color: a.condition === 'above' ? 'var(--green)' : 'var(--red)',
              }}>
                {a.condition === 'above' ? '>' : '<'} ₹{a.target_price.toFixed(2)}
              </span>
              {a.triggered_at && <span style={{ fontSize: 9, color: 'var(--amber)' }}>Triggered</span>}
            </div>
            {a.note && <p style={{ margin: '2px 0 0', fontSize: 10, color: 'var(--text-sub)' }}>{a.note}</p>}
          </div>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <button className={`t-btn t-btn-xs ${a.is_active ? 't-btn-ghost' : ''}`}
              onClick={() => handleToggle(a.id)}
              style={{ fontSize: 9, color: a.is_active ? 'var(--green)' : 'var(--text-sub)' }}>
              {a.is_active ? 'ON' : 'OFF'}
            </button>
