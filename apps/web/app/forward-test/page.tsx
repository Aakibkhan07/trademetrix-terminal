  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 20px', fontFamily: 'var(--font-sans)' }}>
      ...
      <div style={{ marginTop: 40, padding: 20, background: 'var(--panel)', borderRadius: 14, border: '1px solid var(--border-2)' }}>
        ...
        <ul style={{ fontSize: 13, color: 'var(--text-sub)', lineHeight: 1.8, paddingLeft: 18, margin: 0 }}>
  ...
  return (
    <div style={{ background: 'var(--panel)', borderRadius: 14, border: '1px solid var(--border-2)', padding: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        ...
        <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 'var(--radius-pill)', background: 'var(--panel)', color: trendColor, border: '1px solid var(--border-2)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
        {item.status === 'pending' && (
          <button onClick={handleStart} style={{ padding: '7px 18px', borderRadius: 'var(--radius-md)', border: 'none', background: 'var(--gradient-primary)', color: 'var(--text-inverse)', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
            Start Forward Test
          </button>
        )}
        {item.status === 'running' && (
          <button onClick={handleStop} style={{ padding: '7px 18px', borderRadius: 'var(--radius-md)', border: '1px solid var(--red)', background: 'transparent', color: 'var(--red)', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
            Stop
          </button>
        )}
        {(item.status === 'running' || item.status === 'stopped' || item.status === 'completed') && (
          <button onClick={handleCompare} disabled={comparing} style={{ padding: '7px 18px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-2)', background: 'var(--panel)', color: 'var(--text)', fontSize: 13, cursor: comparing ? 'wait' : 'pointer' }}>
        ...
        <div style={{ marginTop: 12, padding: 14, background: 'var(--panel-2)', borderRadius: 10, border: '1px solid var(--border-2)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: (comparison.forward.pnl_pct as number) >= 0 ? 'var(--green)' : 'var(--red)' }}>
            ...
            <div style={{ textAlign: 'center', padding: '8px', borderRadius: 8, background: (comparison.deviation_alert as boolean) ? 'var(--red-dim)' : 'var(--cyan-dim)', border: `1px solid ${(comparison.deviation_alert as boolean) ? 'var(--red-dim)' : 'var(--cyan-dim)'}` }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: (comparison.deviation_alert as boolean) ? 'var(--red)' : 'var(--green)' }}>
