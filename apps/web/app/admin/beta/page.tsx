        ...
        {TABS.map(tab => (
          <button
            ...
            style={{
              ...
              color: activeTab === tab ? 'var(--violet)' : 'var(--text-faint)', cursor: 'pointer', whiteSpace: 'nowrap',
              fontFamily: 'inherit',
            }}
          >
            {tab}
...
function InviteCodesSection() {
  ...
  const handleGenerate = useCallback(() => {
    const generated: InviteCode[] = Array.from({ length: count }, () => ({
      code: generateCode(),
      status: 'available',
    }))
    ...
    <div>
      <div className="t-panel" style={{ padding: 16, marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          <button className="t-btn t-btn-primary t-btn-sm" onClick={handleGenerate} style={{ fontSize: 11 }}>
            Generate Codes
          </button>
        </div>
      </div>
      ...
      {codes.length === 0 ? (
        <EmptyState
          icon="#"
          title="No invite codes generated"
          description="Use the form above to generate new beta invite codes."
        />
      ) : (
          ...
          <table className="t-table" style={{ fontSize: 11, width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(139,92,246,0.12)' }}>
                <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: 'var(--text-faint)', fontSize: 9 }}>CODE</th>
                <th style={{ padding: '8px 12px', textAlign: 'center', fontWeight: 600, color: 'var(--text-faint)', fontSize: 9 }}>STATUS</th>
                <th style={{ padding: '8px 12px', textAlign: 'right', fontWeight: 600, color: 'var(--text-faint)', fontSize: 9 }}>ACTIONS</th>
              </tr>
            </thead>
            <tbody>
              {codes.map(c => (
                <tr key={c.code} style={{ borderBottom: '1px solid rgba(139,92,246,0.06)' }}>
                  <td style={{ padding: '8px 12px', fontFamily: 'var(--font-mono)', fontSize: 12, letterSpacing: '0.05em', userSelect: 'all' }}>
                    {c.code}
                  </td>
                  <td style={{ padding: '8px 12px', textAlign: 'center' }}>
                    <span style={{
                      ...
                      color: c.status === 'available' ? '#22c55e' : c.status === 'used' ? '#f59e0b' : 'var(--red)',
                    }}>
                      <span style={{
                        width: 6, height: 6, borderRadius: '50%',
                        background: c.status === 'available' ? '#22c55e' : c.status === 'used' ? '#f59e0b' : 'var(--red)',
                      }} />
                  ...
                  <td style={{ padding: '8px 12px', textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                      <button className="t-btn t-btn-sm t-btn-ghost" onClick={() => handleCopy(c.code)} style={{ fontSize: 9 }}>
                        Copy
                      </button>
                      {c.status !== 'revoked' && (
                        <button className="t-btn t-btn-sm t-btn-ghost" onClick={() => handleRevoke(c.code)} style={{ fontSize: 9, color: 'var(--red)' }}>
                          Revoke
                        </button>
                      )}
                    </div>
...
function WaitlistSection() {
  ...
  const handleAdd = useCallback(() => {
    ...
    setEntries(prev => [
      ...prev,
      { email: newEmail.trim(), name: newName.trim(), date: new Date().toISOString().slice(0, 10), status: 'Pending' },
    ])
    setNewEmail('')
    setNewName('')
    ...
    <div>
      <div className="t-panel" style={{ padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          <button className="t-btn t-btn-primary t-btn-sm" onClick={handleAdd} style={{ fontSize: 11 }}>
            Add
          </button>
        </div>
      </div>
      ...
      {entries.length === 0 ? (
        ...
        <div className="t-panel" style={{ padding: 0, overflow: 'hidden' }}>
          <table className="t-table" style={{ fontSize: 11, width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(139,92,246,0.12)' }}>
                <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: 'var(--text-faint)', fontSize: 9 }}>NAME</th>
                <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: 'var(--text-faint)', fontSize: 9 }}>EMAIL</th>
                <th style={{ padding: '8px 12px', textAlign: 'center', fontWeight: 600, color: 'var(--text-faint)', fontSize: 9 }}>DATE JOINED</th>
                <th style={{ padding: '8px 12px', textAlign: 'center', fontWeight: 600, color: 'var(--text-faint)', fontSize: 9 }}>STATUS</th>
                <th style={{ padding: '8px 12px', textAlign: 'right', fontWeight: 600, color: 'var(--text-faint)', fontSize: 9 }}>ACTIONS</th>
              </tr>
            </thead>
            <tbody>
              {entries.map(e => (
                <tr key={e.email} style={{ borderBottom: '1px solid rgba(139,92,246,0.06)' }}>
                  ...
                  <td style={{ padding: '8px 12px', color: 'var(--text-faint)' }}>{e.email}</td>
                  <td style={{ padding: '8px 12px', textAlign: 'center', color: 'var(--text-faint)', fontSize: 10 }}>{e.date}</td>
                  <td style={{ padding: '8px 12px', textAlign: 'center' }}>
                    <span style={{
                      display: 'inline-flex', alignItems: 'center', gap: 4,
                      padding: '2px 8px', borderRadius: 4, fontSize: 9, fontWeight: 600,
                      background: e.status === 'Approved' ? 'rgba(34,197,94,0.15)' : e.status === 'Rejected' ? 'rgba(239,68,68,0.15)' : 'rgba(245,158,11,0.15)',
                      color: e.status === 'Approved' ? '#22c55e' : e.status === 'Rejected' ? 'var(--red)' : '#f59e0b',
                    }}>
                      {e.status}
                    </span>
                  </td>
                  <td style={{ padding: '8px 12px', textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                      {e.status === 'Pending' && (
                        <>
                          <button className="t-btn t-btn-sm t-btn-ghost" onClick={() => handleApprove(e.email)} style={{ fontSize: 9, color: '#22c55e' }}>
                            Approve
                          </button>
                          <button className="t-btn t-btn-sm t-btn-ghost" onClick={() => handleReject(e.email)} style={{ fontSize: 9, color: 'var(--red)' }}>
                            Reject
                          </button>
                        </>
                      )}
...
function ApprovalsSection() {
  ...
  const handleSendInvite = useCallback((email: string) => {
    toast('success', `Invite email sent to ${email}`)
  }, [toast])

  const handleRemove = useCallback((email: string) => {
    setApproved(prev => prev.filter(e => e.email !== email))
    toast('info', `Access removed for ${email}`)
  }, [toast])
  ...
  if (approved.length === 0) {
    return (
      <EmptyState
        ...
        description="Approve users from the waitlist to see them here."
      />
    )
  }
      ...
      <table className="t-table" style={{ fontSize: 11, width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ borderBottom: '1px solid rgba(139,92,246,0.12)' }}>
            <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: 'var(--text-faint)', fontSize: 9 }}>NAME</th>
            <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: 'var(--text-faint)', fontSize: 9 }}>EMAIL</th>
            <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: 'var(--text-faint)', fontSize: 9 }}>INVITE CODE</th>
            <th style={{ padding: '8px 12px', textAlign: 'right', fontWeight: 600, color: 'var(--text-faint)', fontSize: 9 }}>ACTIONS</th>
          </tr>
        </thead>
        <tbody>
          {approved.map(a => (
            <tr key={a.email} style={{ borderBottom: '1px solid rgba(139,92,246,0.06)' }}>
              ...
              <td style={{ padding: '8px 12px', color: 'var(--text-faint)' }}>{a.email}</td>
              <td style={{ padding: '8px 12px', fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '0.05em', color: '#22d3ee' }}>
                {a.code}
              </td>
              <td style={{ padding: '8px 12px', textAlign: 'right' }}>
                <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                  <button className="t-btn t-btn-sm t-btn-ghost" onClick={() => handleSendInvite(a.email)} style={{ fontSize: 9 }}>
                    Send Invite Email
                  </button>
                  <button className="t-btn t-btn-sm t-btn-danger" onClick={() => handleRemove(a.email)} style={{ fontSize: 9 }}>
                    Remove Access
                  </button>
                </div>
              </td>
