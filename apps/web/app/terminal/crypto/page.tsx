...
export default function CryptoPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const res = await api.crypto.order({
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, height: '100%', padding: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <h1 style={{ fontFamily: "'DM Sans', 'Inter', sans-serif", fontWeight: 700, fontSize: 18, margin: 0, color: 'var(--text)' }}>Crypto</h1>
          <span style={{
      ...
      <div style={{
        ...
        {pairs.slice(0, 16).map(p => (
          <button key={p.symbol} onClick={() => setSelectedSymbol(p.symbol)} style={{
            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2,
            padding: '6px 12px', borderRadius: 'var(--radius-sm)',
            background: selectedSymbol === p.symbol ? 'var(--bg-active)' : 'var(--bg-tertiary)',
            border: selectedSymbol === p.symbol ? '1px solid rgba(139,92,246,0.3)' : '1px solid var(--border)',
            cursor: 'pointer', flexShrink: 0, minWidth: 90,
            fontFamily: "'DM Sans', 'Inter', sans-serif",
            transition: 'all 120ms ease',
          }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--text)' }}>{p.symbol.replace('USDT', '')}</span>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 700, color: 'var(--text)' }}>
              ${p.price?.toFixed(2) || '—'}
            </span>
            {p.change_pct !== 0 && (
              <span style={{ fontSize: 9, fontWeight: 600, color: (p.change_pct || 0) >= 0 ? 'var(--text-green)' : 'var(--text-red)' }}>
        ...
        <div style={{ width: 300, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div className="t-panel" style={{
            ...
            <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div>
                <label style={{ fontSize: 9, fontWeight: 700, color: 'var(--text-faint)', display: 'block', marginBottom: 3, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Quantity</label>
                <input className="t-input" type="number" min={0} step={0.001} value={qty} onChange={e => setQty(Number(e.target.value))} />
              </div>
              ...
              {orderType === 'limit' && (
                <div>
                  <label style={{ fontSize: 9, fontWeight: 700, color: 'var(--text-faint)', display: 'block', marginBottom: 3, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Limit Price (USD)</label>
                  <input className="t-input" type="number" min={0} step={0.01} value={limitPrice} onChange={e => setLimitPrice(Number(e.target.value))} />
                </div>
              )}
              ...
              {livePair && (
                <div style={{
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--text)', fontSize: 14 }}>
                    ${livePair.price?.toFixed(2) || '—'}
                  </span>
                  {livePair.change_pct !== 0 && (
                    <span style={{ fontWeight: 700, color: (livePair.change_pct || 0) >= 0 ? 'var(--text-green)' : 'var(--text-red)' }}>
                      {(livePair.change_pct || 0) >= 0 ? '+' : ''}{(livePair.change_pct || 0).toFixed(2)}%
                    </span>
                  )}
                </div>
              )}
              ...
              <button onClick={handlePlaceOrder} disabled={placing || !qty} style={{
                ...
                fontFamily: "'DM Sans', sans-serif", transition: 'all 120ms ease',
              }}>
                  ...
                  <tbody>
                    {positions.map((p, i) => (
                      <tr key={i}>
                        ...
                        <td className="t-num">{p.quantity.toFixed(4)}</td>
                        <td className="t-num">${p.entry_price?.toFixed(2) || '—'}</td>
                        <td className="t-num">${p.current_price?.toFixed(2) || '—'}</td>
                  ...
                  <tbody>
                    {orders.slice(0, 50).map((o, i) => (
                      <tr key={i}>
                        ...
                        <td className="t-num">${o.fill_price?.toFixed(2) || '—'}</td>
