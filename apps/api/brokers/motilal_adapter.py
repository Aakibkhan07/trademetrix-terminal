"""Motilal Oswal Securities broker adapter — connect-flow scaffold (credential storage + typed unsupported surface).

Honesty contract: Motilal Oswal Securities (motilaloswalmotilal.com — Motilal Oswal Financial Services Ltd, SEBI INZ000170754) does NOT publish a public trading API today (no developer portal, no API keys). This adapter therefore implements the FULL broker surface as typed failures:
...
_PENDING_API_DETAIL = (
    "Motilal Oswal Securities (motilaloswalmotilal.com) has no public trading API yet — credentials are stored "
    "via the connect flow but live calls stay disabled until Motilal Oswal ships one. "
    "Track https://motilaloswalmotilal.com for API availability."
)

_REQUIRED_CREDENTIALS = ("client_code", "secret_key")


class MotilalOswalAdapter(BaseBroker, BrokerAdapterBase):
    """Motilal Oswal scaffold — stores nothing itself (repo owns encrypted storage);
    every runtime capability is a typed UnsupportedFeatureError until Motilal Oswal
    ...
    async def authenticate(self, credentials: dict) -> Session:
        ...
        if missing:
            raise ValueError(f"Motilal Oswal requires {', '.join(_REQUIRED_CREDENTIALS)} (missing: {', '.join(missing)})")
