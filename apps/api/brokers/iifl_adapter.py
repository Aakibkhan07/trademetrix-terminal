"""IIFL Securities broker adapter — connect-flow scaffold (credential storage + typed unsupported surface).

Honesty contract: IIFL Securities (iiflsecurities.com — IIFL Securities Ltd, SEBI INZ000057540) does NOT publish a public trading API today (no developer portal, no API keys). This adapter therefore implements the FULL broker surface as typed failures:
...
_PENDING_API_DETAIL = (
    "IIFL Securities (iiflsecurities.com) has no public trading API yet — credentials are stored "
    "via the connect flow but live calls stay disabled until IIFL Securities ships one. "
    "Track https://iiflsecurities.com for API availability."
)

_REQUIRED_CREDENTIALS = ("client_code", "secret_key")


class IIFLSecuritiesAdapter(BaseBroker, BrokerAdapterBase):
    """IIFL Securities scaffold — stores nothing itself (repo owns encrypted storage);
    every runtime capability is a typed UnsupportedFeatureError until IIFL Securities
    ...
    async def authenticate(self, credentials: dict) -> Session:
        ...
        if missing:
            raise ValueError(f"IIFL Securities requires {', '.join(_REQUIRED_CREDENTIALS)} (missing: {', '.join(missing)})")
