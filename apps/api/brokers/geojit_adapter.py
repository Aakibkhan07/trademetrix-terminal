"""Geojit broker adapter — connect-flow scaffold (credential storage + typed unsupported surface).

Honesty contract: Geojit (geojit.com — Geojit Financial Services Ltd, SEBI INZ000077175) does NOT publish a public trading API today (no developer portal, no API keys). This adapter therefore implements the FULL broker surface as typed failures:
...
_PENDING_API_DETAIL = (
    "Geojit (geojit.com) has no public trading API yet — credentials are stored "
    "via the connect flow but live calls stay disabled until Geojit ships one. "
    "Track https://geojit.com for API availability."
)

_REQUIRED_CREDENTIALS = ("client_code", "secret_key")


class GeojitAdapter(BaseBroker, BrokerAdapterBase):
    """Geojit scaffold — stores nothing itself (repo owns encrypted storage);
    every runtime capability is a typed UnsupportedFeatureError until Geojit
    ...
    async def authenticate(self, credentials: dict) -> Session:
        ...
        if missing:
            raise ValueError(f"Geojit requires {', '.join(_REQUIRED_CREDENTIALS)} (missing: {', '.join(missing)})")
