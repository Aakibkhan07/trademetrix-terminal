"""OKX broker adapter — connect-flow scaffold (credential storage + typed unsupported surface).

Honesty contract: OKX (okx.com) publishes a public REST API but this platform
...
_PENDING_API_DETAIL = (
    "OKX (okx.com) has a public REST API but is not activated in this "
    ...
    "Track https://okx.com for API availability."
)

_REQUIRED_CREDENTIALS = ("api_key", "secret_key")


class OkxAdapter(BaseBroker, BrokerAdapterBase):
    """OKX scaffold — stores nothing itself (repo owns encrypted storage);
    ...
    async def authenticate(self, credentials: dict) -> Session:
        ...
        if missing:
            raise ValueError(f"OKX requires {', '.join(_REQUIRED_CREDENTIALS)} (missing: {', '.join(missing)})")
