"""Alpaca broker adapter — connect-flow scaffold (credential storage + typed unsupported surface).

Honesty contract: Alpaca (alpaca.markets) publishes a public API for US stocks but
this platform is India-focused; the US-broker integration serves a different market
and requires separate certification. This adapter therefore implements the FULL broker
...
_PENDING_API_DETAIL = (
    "Alpaca (alpaca.markets) has a public API for US stocks but is not activated in "
    "this India-focused platform. Credentials are stored via the connect flow but live "
    "calls stay disabled until a US-broker integration is certified. "
    "Track https://alpaca.markets for API availability."
)

_REQUIRED_CREDENTIALS = ("api_key", "secret_key")


class AlpacaAdapter(BaseBroker, BrokerAdapterBase):
    """Alpaca scaffold — stores nothing itself (repo owns encrypted storage);
    every runtime capability is a typed UnsupportedFeatureError until the US-broker
    ...
    async def authenticate(self, credentials: dict) -> Session:
        ...
        if missing:
            raise ValueError(f"Alpaca requires {', '.join(_REQUIRED_CREDENTIALS)} (missing: {', '.join(missing)})")
