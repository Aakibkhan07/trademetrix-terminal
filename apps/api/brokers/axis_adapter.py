"""Axis Securities broker adapter — connect-flow scaffold (credential storage + typed unsupported surface).

Honesty contract: Axis Securities (axissecex.com — Axis Securities Ltd, SEBI INZ000077287) does NOT publish a public trading API today (no developer portal, no API keys). This adapter therefore implements the FULL broker surface as typed failures:
...
_PENDING_API_DETAIL = (
    "Axis Securities (axissecex.com) has no public trading API yet — credentials are stored "
    "via the connect flow but live calls stay disabled until Axis Securities ships one. "
    "Track https://axissecex.com for API availability."
)

_REQUIRED_CREDENTIALS = ("client_code", "secret_key")


class AxisSecuritiesAdapter(BaseBroker, BrokerAdapterBase):
    """Axis Securities scaffold — stores nothing itself (repo owns encrypted storage);
    every runtime capability is a typed UnsupportedFeatureError until Axis Securities
    ...
    async def authenticate(self, credentials: dict) -> Session:
        ...
        if missing:
            raise ValueError(f"Axis Securities requires {', '.join(_REQUIRED_CREDENTIALS)} (missing: {', '.join(missing)})")
