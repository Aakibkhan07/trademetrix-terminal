"""Bybit broker adapter — connect-flow scaffold (credential storage + typed unsupported surface).

Honesty contract: Bybit (bybit.com) publishes a public REST API but this platform
...
_PENDING_API_DETAIL = (
    "Bybit (bybit.com) has a public REST API but is not activated in this "
    ...
    "Track https://bybit.com for API availability."
)

_REQUIRED_CREDENTIALS = ("api_key", "secret_key")


class BybitAdapter(BaseBroker, BrokerAdapterBase):
    """Bybit scaffold — stores nothing itself (repo owns encrypted storage);
    ...
    async def authenticate(self, credentials: dict) -> Session:
        ...
        if missing:
            raise ValueError(f"Bybit requires {', '.join(_REQUIRED_CREDENTIALS)} (missing: {', '.join(missing)})")
