"""Reliance Securities broker adapter — connect-flow scaffold (credential storage + typed unsupported surface).

Honesty contract: Reliance Securities (reliancesecurities.com — Reliance Securities Ltd, SEBI INZ000077287) does NOT publish a public trading API today (no developer portal, no API keys). This adapter therefore implements the FULL broker surface as typed failures:
...
_PENDING_API_DETAIL = (
    "Reliance Securities (reliancesecurities.com) has no public trading API yet — credentials are stored "
    "via the connect flow but live calls stay disabled until Reliance Securities ships one. "
    "Track https://reliancesecurities.com for API availability."
)

_REQUIRED_CREDENTIALS = ("client_code", "secret_key")


class RelianceSecuritiesAdapter(BaseBroker, BrokerAdapterBase):
    """Reliance Securities scaffold — stores nothing itself (repo owns encrypted storage);
    every runtime capability is a typed UnsupportedFeatureError until Reliance Securities
    publishes a trading API."""

    broker_name = "reliance"
    ...
    async def authenticate(self, credentials: dict) -> Session:
        ...
        if missing:
            raise ValueError(f"Reliance Securities requires {', '.join(_REQUIRED_CREDENTIALS)} (missing: {', '.join(missing)})")
