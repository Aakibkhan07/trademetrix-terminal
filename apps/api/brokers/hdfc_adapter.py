"""HDFC Securities broker adapter — connect-flow scaffold (credential storage + typed unsupported surface).

Honesty contract: HDFC Securities (hdfcsec.com — HDFC Bank Ltd, SEBI INZ000123234) does NOT publish a public trading API today (no developer portal, no API keys). This adapter therefore implements the FULL broker surface as typed failures:
...
_PENDING_API_DETAIL = (
    "HDFC Securities (hdfcsec.com) has no public trading API yet — credentials are stored "
    "via the connect flow but live calls stay disabled until HDFC Securities ships one. "
    "Track https://hdfcsec.com for API availability."
)

_REQUIRED_CREDENTIALS = ("client_code", "secret_key")


class HDFCSecuritiesAdapter(BaseBroker, BrokerAdapterBase):
    """HDFC Securities scaffold — stores nothing itself (repo owns encrypted storage);
    every runtime capability is a typed UnsupportedFeatureError until HDFC Securities
    ...
    async def authenticate(self, credentials: dict) -> Session:
        ...
        if missing:
            raise ValueError(f"HDFC Securities requires {', '.join(_REQUIRED_CREDENTIALS)} (missing: {', '.join(missing)})")
