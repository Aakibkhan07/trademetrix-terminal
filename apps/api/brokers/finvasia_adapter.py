...
class FinvasiaAdapter(BaseBroker, BrokerAdapterBase):
    ...
    def __init__(self):
        ...
        self._user_id: str = ""
        self._base_url = "https://api.shoonya.com/NorenWClientTP"
        self._ws_url = "wss://api.shoonya.com/NorenWSTP"
        self._running = False
    ...
    async def authenticate(self, credentials: dict) -> Session:
        ...
        vc = credentials.get("additional_params", {}).get("vendor_code", "SHOONYA_ABHI_11")

        self._user_id = uid
        self._access_token = credentials.get("access_token") or ""

        if not self._access_token and uid and pwd and factor2:
            apk = ""
            imei = "1234567890"
        ...
        if not self._access_token:
            raise ValueError("access_token required for Shoonya authentication")
    ...
    def _parse_tick(self, data: dict) -> Tick | None:
        try:
            ...
            logger.warning("Failed to parse Shoonya tick: %s", e)
            return None
