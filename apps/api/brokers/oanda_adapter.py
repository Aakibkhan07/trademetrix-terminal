"""OANDA broker adapter — connect-flow scaffold (credential storage + typed unsupported surface).

Honesty contract: OANDA (oanda.com) publishes a public API for forex but this platform
is India-focused; forex/CFD integrations serve a different market and require separate
certification. This adapter therefore implements the FULL broker surface as typed failures:
...
_PENDING_API_DETAIL = (
    "OANDA (oanda.com) has a public API for forex/CFD but is not activated in this "
    "India-focused platform. Credentials are stored via the connect flow but live "
    "calls stay disabled until a forex integration is certified. "
    "Track https://oanda.com for API availability."
)

_REQUIRED_CREDENTIALS = ("api_key",)


class OandaAdapter(BaseBroker, BrokerAdapterBase):
    """OANDA scaffold — stores nothing itself (repo owns encrypted storage);
    every runtime capability is a typed UnsupportedFeatureError until the forex
    ...
    async def authenticate(self, credentials: dict) -> Session:
        ...
        if missing:
            raise ValueError(f"OANDA requires {', '.join(_REQUIRED_CREDENTIALS)} (missing: {', '.join(missing)})")
