"""Interactive Brokers broker adapter — connect-flow scaffold (credential storage + typed unsupported surface).

Honesty contract: Interactive Brokers (interactivebrokers.com) offers a global API
(TWS / Client Portal) but this platform is India-focused; the global broker integration
serves a different market and requires separate certification. This adapter therefore
implements the FULL broker surface as typed failures:
...
_PENDING_API_DETAIL = (
    "Interactive Brokers (interactivebrokers.com) offers a global API (TWS / "
    "Client Portal) but is not activated in this India-focused platform. Credentials "
    "are stored via the connect flow but live calls stay disabled until a global "
    "broker integration is certified. Track https://interactivebrokers.com for "
    "API availability."
)
...
class InteractiveBrokersAdapter(BaseBroker, BrokerAdapterBase):
    """Interactive Brokers scaffold — stores nothing itself (repo owns encrypted
    storage); every runtime capability is a typed UnsupportedFeatureError until the
    global broker integration is certified."""
    ...
    async def authenticate(self, credentials: dict) -> Session:
        ...
        if missing:
            raise ValueError(f"Interactive Brokers requires {', '.join(_REQUIRED_CREDENTIALS)} (missing: {', '.join(missing)})")
