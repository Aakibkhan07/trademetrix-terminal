...
async def admin_users_with_brokers(
    admin: UserProfile = Depends(require_super_admin),
):
...
class AddWhitelistIPRequest(BaseModel):
    ip_address: str
    label: str = ""


@router.get("/ip-whitelist")
async def admin_list_ip_whitelist(
    admin: UserProfile = Depends(require_super_admin),
):
...
async def admin_add_ip_whitelist(
    req: AddWhitelistIPRequest,
    admin: UserProfile = Depends(require_super_admin),
):
    return await _service.add_ip_whitelist(req.ip_address.strip(), req.label.strip(), admin.id)
