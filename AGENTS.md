...
## Session: 2026-08-03 — Production Readiness v1.1.0 + hotfixes (live-data pages, full E2E)
...
### Reference
- Health/metrics mount at API root: `/health`, `/health/ready`, `/health/metrics` — NEVER add `/api/v1` when probing from web; use `API_ORIGIN` derived from `NEXT_PUBLIC_API_URL`.
- `core/deps.py` caches profiles 120s (`_user_cache`) — ANY write to `profiles` must pop the cache or `/auth/me` goes stale. Same for future profile mutations.
- Onboarding PATCH needs the `onboarding_completed` column: `ALTER TABLE profiles ADD COLUMN IF NOT EXISTS onboarding_completed BOOLEAN NOT NULL DEFAULT false;` (already applied remotely).
- FastAPI 0.141.x lazy `_IncludedRouter`: routes expand on first request per router — `_prewarm_lazy_routes(app)` in `main.py` forces expansion at import.
- Curl CSRF flow: `-c jar` on `/api/v1/auth/csrf` GET, then `-b jar -c jar` on POSTs; the `-d` flag defaults method to POST — always pair with `-X PATCH` etc.
- API regression: `.venv/bin/python -m pytest tests/` → 563 passed, 1 xfailed (bare `pytest` collects root `pat_test.py` → sys.exit).
